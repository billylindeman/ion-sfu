...
type Signal struct {
	...
	config sfu.SignalConfig
}

// NewSignal creates a signaling server
func NewSignal(s *sfu.SFU, c sfu.Coordinator, conf sfu.SignalConfig) (*Signal, chan error) {
	e := make(chan error)
	w := &Signal{
		sfu:     s,
		c:       c,
...
func (s *Signal) ServeWebsocket() {
	...
	r.Handle("/session/{id}", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if meta.Redirect {
			endpoint := fmt.Sprintf("%v/session/%v", meta.NodeURL, meta.SessionID)
