...
import (
	...
	cluster "github.com/pion/ion-sfu/cmd/cluster/cluster"
...
func main() {
	...
	coordinator, err := sfu.NewCoordinator(conf)
	if err != nil {
		log.Errorf("error configuring coordinator: %v", err)
	}
	sfu := sfu.NewSFU(conf)

	// Spin up websocket
	sServer, sError := cluster.NewSignal(sfu, coordinator, conf.Signal)
